...
for bulan in daftarbulan:
    ...
    data  = pd.read_excel('F:/GRAFIK PASUT SEMARANG 2022/MASTER GRAFIK PASUT 2022  SAMPIT-NEW.xls', 
        sheet_name=list_sheet.get(bulan),skiprows=5,skipfooter=4, header=None, usecols='B:Y')
    ...
    for tanggal,tgl in zip(list_tanggal,bln):
        ...
        dir_hasil = 'F:/GRAFIK SAMPIT 2022 PUSHIDROSAL/{0}/'.format(list_dir.get(bulan))
        if not os.path.exists(dir_hasil):
            os.makedirs(dir_hasil)
        msl = np.full((24,),1.7)
        y = data.iloc[tgl,:]
        x = np.arange(1,25)
        y_tick = [0.2,0.7,1.2,1.7,2.2,2.7,3.2]
